...
Uint16 BreakCloseNo1ToNo2(void)
{
	...
	if (flag == 1)
	{
		...
		{
			if (SpllRegUFO.ThetaA == 78)										// 未追踪到同步，分相投入
			{
				CtrlSCREnable(No2AB);
			...
			else if (SpllRegUFO.ThetaB == 78)                                        // 未追踪到同步，分相投入
            {
                CtrlSCREnable(No2BC);
			...
			else if (SpllRegUFO.ThetaC == 78)                                        // 未追踪到同步，分相投入
            {
                CtrlSCREnable(No2CA);
...
Uint16 BreakCloseNo2ToNo1(void)
{
	...
	if (flag == 1)
	{
		...
		{
			if (SpllRegUFO.ThetaA == 77)										// 未追踪到同步，分相投入
			{
				CtrlSCREnable(No1AB);
				FlagSystem.bit.ABChannelOn = 1;
				flag = 0;
			...
			else if (SpllRegUFO.ThetaB == 77)                                        // 未追踪到同步，分相投入
            {
                CtrlSCREnable(No1BC);
                FlagSystem.bit.BCChannelOn = 1;
                flag = 0;
            ...
            else if (SpllRegUFO.ThetaC == 77)                                        // 未追踪到同步，分相投入
            {
                CtrlSCREnable(No1CA);
                FlagSystem.bit.ACChannelOn = 1;
                flag = 0;
